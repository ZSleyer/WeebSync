...
describe('upcomingAirings', () => {
  ...
  it('flattens every release across the watches, oldest first', () => {
    ...
    expect(list[1].episodeAbs).toBe(1112)
  })

  it('caps the horizon in days when asked', () => {
    const far = watch(4, [{ at: 1_000_000 + 8 * 86_400, episode: 1 }])
    expect(upcomingAirings([...ws, far], now, 7).map((e) => e.watch.id)).toEqual([1, 2, 1])
    expect(upcomingAirings([...ws, far], now).map((e) => e.watch.id)).toEqual([1, 2, 1, 4])
  })
