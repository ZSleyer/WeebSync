...
const d: WatchDefaults = {
  kinds: {
    'anime-series': {
      localPath: 'Anime',
      subfolder: false,
      subfolderSource: 'title',
      subfolderSeparator: '_',
      template: '{title} - {episode:02}',
      separator: '.',
    },
    movie: {
      localPath: 'Filme',
  ...
  common: {
    renameProvider: 'tvdb',
    renameOrdering: 'official',
    renameTitleLang: 'de-DE',
    airedMapping: true,
    wantDub: 'Ger',
    wantSub: '',
    dubLagDays: 0,
...
describe('applyDefaults', () => {
  it('fills the kind folder and naming when nothing was chosen, plus the common blanks', () => {
    ...
    expect([f.renameProvider, f.renameOrdering, f.renameTitleLang, f.airedMapping, f.wantDub, f.plexSubLang]).toEqual([
      'tvdb',
      'official',
      'de-DE',
      true,
  ...
  it('keeps a folder a plan already found and a value already set', () => {
    const f = applyDefaults(
      { ...blank, localPath: '/lib/Frieren/Season 1', template: '{title} - S01E{episode:02}', wantDub: 'Jap' },
      'anime-series',
      d,
    )
    expect([f.localPath, f.subfolder, f.template, f.wantDub]).toEqual([
      '/lib/Frieren/Season 1',
      false,
      '{title} - S01E{episode:02}',
      'Jap',
    ])
  })
  ...
  it('sends a season to the library folder the show already has, path and template alike', () => {
    const lib = {
      ...
      libraryDir: '/lib/Frieren',
    }
    const f = applyDefaults(blank, 'anime-series', { ...d, common: { ...d.common, airedMapping: false } }, lib)
    expect([f.localPath, f.subfolderSource, f.seasonFolder, f.template]).toEqual([
      '/lib/Frieren/Season 2',
      'none',
      'Season 2',
      '{title} - {episode:02}',
    ])
    ...
    const h = applyDefaults(
      { ...blank, template: '{title} - S{season:02}E{episode:02}' },
      'anime-series',
      undefined,
      lib,
    )
  ...
  it('hands the season folder to a title subfolder and keeps it out of a folder template', () => {
    ...
    const folders = {
      ...
      kinds: {
        'anime-series': {
          ...d.kinds['anime-series']!,
          template: 'Season {season:02}/{title} - S{season:02}E{episode:02}',
        },
      },
    }
    const g = applyDefaults(
      blank,
      'anime-series',
      { ...folders, common: { ...d.common, airedMapping: false } },
      { ...sub, libraryDir: '/lib/Frieren' },
    )
    expect([g.localPath, g.seasonFolder, g.template]).toEqual([
      '/lib/Frieren',
      undefined,
      'Season {season:02}/{title} - S{season:02}E{episode:02}',
    ])
  })
  it('maps suggestion categories onto kinds', () => {
    expect(['anime-tv', 'anime-movie', 'animation-tv', 'movie', 'tv'].map(suggestionKind)).toEqual([
      'anime-series',
      'anime-movie',
      'series',
      'movie',
