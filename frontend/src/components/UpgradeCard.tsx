...
export default function UpgradeCard({
  ...
  const { t } = useTranslation()
  const seasonLabel = u.isMovie
    ? t('suggestions.movie')
    : u.season > 0
      ? t('suggestions.season', { season: u.season })
      : ''
  ...
  const syncInfo = [
    t('watch.infoSource', {
      server: chosen.serverName || t('suggestions.localPlex'),
      quality: variantQuality(chosen, t),
    }),
  ...
  return (
    ...
    <Panel className="flex flex-wrap items-start gap-4 p-3">
      {u.media && onDetails ? (
        <button
          type="button"
          aria-label={t('remote.detailsFor', { name: u.title })}
          onClick={() => onDetails(u)}
                        ...
                        {g.items.map((o, j) => {
                          ...
                          return (
                            <li key={`${o.serverId}-${o.folder}-${j}`} className="min-w-0">
                              ...
                              >
                                <Radio name={`opt-${u.key}`} checked={isChosen(o)} onChange={() => onChoose(o)} />
            ...
            <Button
              ...
              onClick={() =>
                onSync({
                  ...
                  name: u.title,
                  initial: syncFields(u.sync!, u.title, chosen.folder),
                  info: syncInfo,
                })
              }
            >
