  ...
  return (
    <div>
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="font-display text-xl font-semibold tracking-wider">DASHBOARD</h2>
          <span className="t-label mt-1">transfer queue</span>
        </div>
        <div className="flex gap-3">
          <StatTile label="aktiv" value={String(active.filter((d) => d.status === 'running').length)} />
          <StatTile label="queue" value={String(active.filter((d) => d.status === 'queued').length)} />
          <StatTile label="speed" value={fmtSpeed(totalSpeed)} wide>
            <SpeedSparkline current={totalSpeed} />
          </StatTile>
        </div>
      </header>

      <section aria-label="Aktive Downloads">
        {active.length === 0 && (
          <div className="t-panel p-8 text-center text-t-muted">
            Keine aktiven Downloads. Ab in den <span className="text-accent">Browser</span> zum Syncen.
          </div>
        )}
      ...
      {finished.length > 0 && (
        <section aria-label="Abgeschlossene Downloads" className="mt-8">
          <span className="t-label mb-3">history</span>
          <div className="mt-2 flex flex-col gap-2">
            {finished.slice(0, 20).map((d) => (
              <div key={d.id} className="flex items-center gap-3 border border-border-subtle bg-bg-card px-3 py-2 text-sm">
                ...
                <span className="font-mono text-xs text-t-faint">{fmtBytes(d.size)}</span>
                ...
                <button
                  ...
                  aria-label={`Eintrag ${d.id} entfernen`}
...
function SpeedSparkline({ current }: { current: number }) {
  ...
  return (
    <svg width={w} height={h} className="mb-1 shrink-0" role="img" aria-label="Gesamtgeschwindigkeit, letzte 60 Sekunden">
      {hist.length > 1 && (
...
export function StatusChip({ status }: { status: Download['status'] }) {
  ...
  return <span className={`t-label ${cls}`}>{status}</span>
}
  ...
  return (
    <div className="t-panel p-4">
      ...
      <div
        ...
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={`Fortschritt ${d.remotePath.split('/').pop()}`}
      >
        <div
